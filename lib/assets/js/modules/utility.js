...
class Utility {
  ...
  static slideDown($target, callback = function() {}, duration = 200) {
    ...
    let targetHeight;
    let targetPaddingTop;
    let targetPaddingBottom;
    let startTime;
    $target.style.height = '';
    $target.style.overflow = 'hidden';
    ...
    targetPaddingTop = parseInt(getComputedStyle($target).getPropertyValue('padding-top'));
    targetPaddingBottom = parseInt(getComputedStyle($target).getPropertyValue('padding-bottom'));
    $target.style.overflow = 'hidden';
    $target.style.height = '0px';
  ...
  static throttle(func, interval = 50, isLastExec = false) {
