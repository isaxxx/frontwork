  ...
  return new Promise((resolve) => {
    ...
    if (vendorFilesPath.length) {
      ...
      vendorFilesPath.forEach((vendorFilePath) => {
        processing.push(new Promise((resolve, reject) => {
          const destFilePath = './dest/' + config.path.assets + '/' + config.path.css + '/' + path.basename(vendorFilePath);
          cpx.copy(vendorFilePath, './dest/' + config.path.assets + '/' + config.path.css + '/', (err) => {
            if (err) {
              reject(err);
            } else {
              console.log(chalk.green('Output: ' + destFilePath));
